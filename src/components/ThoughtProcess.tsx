...
export const ThoughtProcess: React.FC<ThoughtProcessProps> = ({ thoughts, isStreaming }) => {
  ...
  return (
    <div className="mb-4 rounded-xl border border-slate-200 bg-slate-50 overflow-hidden text-xs">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full px-3.5 py-2.5 flex items-center justify-between bg-slate-100/80 hover:bg-slate-200/60 transition-colors text-slate-800 font-medium cursor-pointer"
      >
        <div className="flex items-center gap-2">
          ...
          <span>Tiến Trình Tra Cứu & Suy Luận ({thoughts.length} bước)</span>
        </div>
      ...
      {isOpen && (
        <div className="p-3 space-y-2 bg-white border-t border-slate-200">
          {thoughts.map((t, idx) => {
            ...
            return (
              <div key={idx} className="flex flex-col gap-1.5 p-2.5 rounded-lg bg-slate-50 border border-slate-200/80">
