...
export const ChatWindow: React.FC<ChatWindowProps> = ({
  ...
  return (
    <div className="flex flex-col h-full rounded-2xl bg-white border border-slate-200/80 shadow-sm overflow-hidden">
      {/* Header */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-left">
                {quickPrompts.map((prompt, idx) => (
                  <button
                    ...
                    className="p-2.5 rounded-xl bg-white border border-slate-200 hover:border-blue-500 text-xs text-slate-700 hover:text-blue-600 transition-all text-left shadow-2xs cursor-pointer flex items-center gap-2.5 group"
                  >
                    <div className="p-1 rounded-lg bg-blue-50 text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-colors shrink-0">
                ...
                >
                  ...
                  {msg.content ? (
                    <div className="space-y-3">
                      <div className="prose prose-slate prose-xs max-w-none prose-p:leading-relaxed">
                        ...
                        <div className="mt-3 pt-2.5 border-t border-slate-100 space-y-1.5">
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {msg.sources.map((src, sIdx) => (
                              <div
                                key={sIdx}
                                className="px-2.5 py-1 rounded-lg bg-slate-100 text-slate-700 text-[11px] font-medium border border-slate-200 flex items-center gap-1.5 shadow-2xs hover:border-slate-300 transition-colors"
                              >
      ...
      {/* Input Form */}
      <form onSubmit={handleSubmit} className="p-3 sm:p-3.5 border-t border-slate-100 bg-white space-y-2">
        <div className="flex items-center gap-2 bg-slate-50 focus-within:bg-white focus-within:ring-2 focus-within:ring-blue-500/40 rounded-xl p-2 transition-all border border-slate-200">
          <input
            type="text"
            value={inputPrompt}
