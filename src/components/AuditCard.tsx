...
export const AuditCard: React.FC<AuditCardProps> = ({ report }) => {
  ...
  return (
    <div className={`my-4 rounded-xl border p-4 transition-all shadow-sm ${badge.bg}`}>
      {/* Header */}
      <div className="flex items-center justify-between border-b border-slate-200/80 pb-2.5 mb-3">
        ...
        {report.variancePercentage !== undefined && report.variancePercentage !== null && (
          <span className="inline-flex items-center gap-1 text-xs font-mono px-2 py-0.5 rounded bg-white border border-slate-200 text-slate-800 font-semibold shadow-sm">
            <TrendingDown className="w-3 h-3 text-red-600" />
            Chênh lệch: {report.variancePercentage.toFixed(1)}% ({report.varianceUsd ? `$${report.varianceUsd.toLocaleString()}` : ''})
          </span>
        )}
      </div>
